...
.opt-title { font-size: 14px; font-weight: 700; color: #1a1a2e; margin-bottom: 3px; }
.opt-sub { font-size: 12px; color: rgba(26,26,46,0.45); line-height: 1.4; }
...
.hl {
    background: linear-gradient(135deg, #6366F1 0%, #F5C518 55%, #FF6B35 100%);
...
/* ── Checkbox ── */
.stCheckbox > label { color: rgba(26,26,46,0.75) !important; font-size: 14px !important; }

/* ── Radio ── */
.stRadio > label { color: rgba(26,26,46,0.65) !important; }
...
/* ── slider ── */
.stSlider > div { color: rgba(26,26,46,0.65) !important; }
...
def render_nav():
    ...
    <div class="navbar">
        ...
        <div style="font-size:12px;color:rgba(26,26,46,0.3);font-weight:500;letter-spacing:0.3px;">
            每个人都值得找到属于自己的圈子
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def tags_html(club):
    ...
    for t in club.get("tags", []):
        h += f'<span class="badge badge-w">{t}</span>'
    return h
...
def local_score(answers):
    ...
    for c in CLUBS:
        ...
        a7 = answers.get(7)
        if a7 == 0 and c["id"] in [10, 12]: s += 5
        if a7 == 1 and c["id"] in [11, 13]: s += 5
        if a7 == 2 and c["id"] in [7, 15]: s += 5
        if a7 == 3 and c["id"] in [9, 14, 3]: s += 5
...
def ai_match(answers):
    try:
        ...
        prompt = f"""你是大学社团匹配顾问。根据用户测评，为{len(CLUBS)}个社团打匹配分（0-100整数），给出最高匹配社团的一句话原因（温暖人性化，不超过40字）。
...
风格：简洁、温暖、真实，给具体有用的建议，不废话，用中文，适当换行，不用过多 emoji。
...
def page_home():
    ...
    with col_l:
        ...
        for col, (n, l) in zip([c1, c2, c3, c4],
            ...
            with col:
                st.markdown(
                    f'<div style="text-align:center;"><div class="stat-num">{n}</div><div class="stat-lbl">{l}</div></div>',
                    unsafe_allow_html=True)
    ...
    with col_r:
        ...
        <div class="glass featured" style="margin-top:4px;">
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
                {''.join([f'<div style="background:rgba(99,102,241,0.05);border:1px solid rgba(99,102,241,0.1);border-radius:12px;padding:10px;text-align:center;"><div style="font-size:14px;font-weight:800;color:#1a1a2e;">{v}</div><div style="font-size:10px;color:rgba(26,26,46,0.35);margin-top:2px;">{l}</div></div>' for v, l in [(top['members'], '成员'), ('⭐' + str(top['rating']), '评分'), (str(top['awards']) + '项', '获奖')]])}
            </div>
        </div>
    ...
    st.markdown(
        '<div style="text-align:center;margin-bottom:28px;"><div style="font-size:11px;font-weight:700;letter-spacing:1.2px;color:#6366F1;text-transform:uppercase;margin-bottom:10px;">我们不一样</div><div style="font-size:26px;font-weight:900;letter-spacing:-0.8px;color:#1a1a2e;">不是随机，是属于你的那个</div></div>',
        unsafe_allow_html=True)

    cols = st.columns(3, gap="medium")
    features = [
        ("🧬", "真实的你，真实的匹配",
         "我们问的不是「你有什么爱好」，而是「你周末真的会做什么」——8 道情景题，比你自己更了解你适合哪种氛围。"),
        ("📊", "社团数据全透明",
         "成员活跃度、时间投入、历年评价……在你加入之前，我们让数据替你把关。16 个社团，深度介绍。"),
        ("🤝", "加入后，我们也在",
         "申请提交只是开始。我们会告诉你接下来发生什么、怎么融入、社团群如何运营——让你不迷茫。"),
    ]
    ...
    st.markdown(
        '<div style="font-size:11px;font-weight:700;letter-spacing:1.2px;color:#6366F1;text-transform:uppercase;margin-bottom:10px;">✦ 今日精选</div>',
        unsafe_allow_html=True)
    st.markdown(
        '<div style="font-size:22px;font-weight:900;letter-spacing:-0.6px;margin-bottom:20px;color:#1a1a2e;">随便逛逛，说不定就遇上了</div>',
        unsafe_allow_html=True)
...
def page_quiz():
    ...
    with mid:
        st.markdown(
            f'<div style="font-size:11px;font-weight:700;color:#6366F1;letter-spacing:0.8px;text-transform:uppercase;margin-bottom:10px;">第 {step + 1} 题</div>',
            unsafe_allow_html=True)
        st.markdown(
            f'<div style="font-size:clamp(20px,3vw,28px);font-weight:900;letter-spacing:-0.8px;line-height:1.25;margin-bottom:6px;color:#1a1a2e;">{q["q"]}</div>',
            unsafe_allow_html=True)
        if q.get("hint"):
            st.markdown(
                f'<div style="font-size:13px;color:rgba(26,26,46,0.32);margin-bottom:22px;font-style:italic;">{q["hint"]}</div>',
                unsafe_allow_html=True)
        else:
        ...
        for i, (col, opt) in enumerate(zip(all_cols, q["opts"])):
            with col:
                is_sel = cur == i
                sel_border = "border: 2px solid #6366F1; background: rgba(99,102,241,0.08);" if is_sel else ""
                # 视觉卡片（不可点击，仅展示）
                st.markdown(f"""
                <div class="opt-display {'sel' if is_sel else ''}" style="{sel_border}">
                ...
                """, unsafe_allow_html=True)
                # 可点击按钮，文案与卡片对应
                btn_label = f"✓  {opt['title']}" if is_sel else opt['title']
                if st.button(btn_label, key=f"q{step}_o{i}",
...
def page_results():
    ...
    if reason:
        st.markdown(f"""
        <div style="background:rgba(99,102,241,0.06);border:1px solid rgba(99,102,241,0.15);border-radius:14px;padding:14px 18px;font-size:14px;color:rgba(26,26,46,0.75);line-height:1.65;margin-bottom:24px;">
        ...
        with cols[idx % 3]:
            ...
            with b1:
                if st.button("查看详情", key=f"r_detail_{club['id']}_{idx}", use_container_width=True):
                    st.session_state.expand_club = club["id"] if st.session_state.expand_club != club[
                        "id"] else None
                    st.rerun()
            with b2:
                if applied:
                    st.button("已报名 ✓", key=f"r_applied_{club['id']}", disabled=True,
                              use_container_width=True)
                elif in_c:
                    if st.button("移出申请袋", key=f"r_rm_{club['id']}_{idx}", use_container_width=True):
                        toggle_cart(club["id"]); st.rerun()
                else:
                    if st.button("➕ 加入申请袋", key=f"r_add_{club['id']}_{idx}", type="primary",
                                 use_container_width=True):
                        toggle_cart(club["id"]); st.rerun()
    ...
    with c4:
        ...
        if cart_n:
            if st.button(f"📋 提交申请 ({cart_n})", key="r_apply", type="primary",
                         use_container_width=True): go("apply")
        else:
...
def render_club_detail(club):
    ...
    <div class="detail-box">
        ...
        {''.join([f'<div class="req-row"><span style="color:#6366F1;margin-top:2px;flex-shrink:0;">·</span><span>{r}</span></div>' for r in club.get('requirements', [])])}
        <div class="detail-section-title">主要活动</div>
        {''.join([f'<div class="req-row"><span style="color:#F97316;margin-top:2px;flex-shrink:0;">·</span><span>{a}</span></div>' for a in club.get('activities', [])])}
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:14px;">
            {''.join([f'<div style="background:rgba(99,102,241,0.05);border:1px solid rgba(99,102,241,0.08);border-radius:10px;padding:10px;text-align:center;"><div style="font-size:14px;font-weight:800;color:#1a1a2e;">{v}</div><div style="font-size:10px;color:rgba(26,26,46,0.35);margin-top:2px;">{l}</div></div>' for v, l in [(str(club['members']), '成员'), ('⭐' + str(club['rating']), '评分'), (str(club['awards']) + '项', '获奖'), (club['time_cost'], '时间投入')]])}
        </div>
        {'<div style="margin-top:12px;padding:10px 14px;background:rgba(74,222,128,0.08);border:1px solid rgba(74,222,128,0.2);border-radius:10px;font-size:12px;color:#15803d;text-align:center;">✓ 你已报名这个社团</div>' if applied else ''}
    </div>
    """, unsafe_allow_html=True)
...
def page_browse():
    ...
    with col_s:
        search = st.text_input("", placeholder="🔍  搜索名称、类型、标签...", label_visibility="collapsed",
                               key="b_search")
    with col_f:
    ...
    filtered = [
        c for c in CLUBS
        if (not search or search in c["name"] or search in c["type"] or any(
            search in t for t in c.get("tags", [])) or search in c["desc"])
        ...
        with cols[idx % 3]:
            ...
            <div class="glass">
                ...
                <div style="display:grid;grid-template-columns:repeat(3,1fr);border-top:1px solid rgba(99,102,241,0.07);padding-top:12px;">
                    {''.join([f'<div style="text-align:center;"><div style="font-size:13px;font-weight:800;color:#1a1a2e;">{v}</div><div style="font-size:10px;color:rgba(26,26,46,0.35);margin-top:2px;">{l}</div></div>' for v, l in [(str(club['members']), '成员'), ('⭐' + str(club['rating']), '评分'), (str(club['awards']) + '项', '获奖')]])}
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with b1:
                ...
                if st.button("收起 ↑" if exp else "查看详情", key=f"b_det_{club['id']}_{idx}",
                             use_container_width=True):
            ...
            with b2:
                if applied:
                    ...
                    if st.button("➕ 加入申请袋", key=f"b_add_{club['id']}_{idx}", type="primary",
                                 use_container_width=True):
                        toggle_cart(club["id"]); st.rerun()
    ...
    with mid:
        cart_n = len(st.session_state.cart)
        if cart_n:
            if st.button(f"📋  前往提交申请（已选 {cart_n} 个）", type="primary", use_container_width=True,
                         key="b_go_apply"):
                go("apply")
...
def page_chat():
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.markdown(
        '<div style="font-size:13px;color:rgba(26,26,46,0.38);margin-bottom:24px;">由 Claude AI 驱动 · 随时回答关于社团的任何疑问</div>',
        unsafe_allow_html=True)

    if not st.session_state.chat_history:
        st.session_state.chat_history = [{"user": None,
                                           "ai": f"嗨！我是 ClubMatch AI 顾问 👋<br /><br />我对平台上所有 {len(CLUBS)} 个社团都了如指掌。你可以问我：<br />· 某个社团的具体情况和氛围<br />· 根据你的情况给个性化推荐<br />· 「内向的人适合哪个」「我时间不多怎么选」……<br /><br />随便问吧，没有奇怪的问题。"}]
    ...
    if len(st.session_state.chat_history) <= 1:
        st.markdown(
            ...
            unsafe_allow_html=True)
        sugg = ["内向的人适合哪个社团？", "我时间不多，选哪个好？", "摄影社和微电影社有啥区别？",
                "创业社真的有用吗？", "不知道自己喜欢什么怎么办？", "哪个社团最容易交到朋友？"]
        cols = st.columns(3, gap="small")
    ...
    with col_i:
        user_input = st.text_input("", placeholder="问我任何关于社团的事...", label_visibility="collapsed",
                                   key="chat_inp")
    with col_s:
...
def page_apply():
    ...
    if not cart:
        ...
        st.markdown(
            ...
            unsafe_allow_html=True)
        st.markdown(
            '<div style="color:rgba(26,26,46,0.4);margin-bottom:24px;font-size:14px;">先去发现社团，把感兴趣的加进申请袋再来这里～</div>',
            unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.markdown(
        f'<div style="font-size:14px;color:rgba(26,26,46,0.4);margin-bottom:24px;">你选择了 {len(cart)} 个社团，一次填写全搞定</div>',
        unsafe_allow_html=True)
            ...
            <div style="display:flex;flex-direction:column;gap:12px;">
                ...
                <div style="display:flex;align-items:flex-start;gap:12px;">
                    ...
                    <div>
                        ...
                        <div style="font-size:12px;color:rgba(26,26,46,0.45);line-height:1.5;">确认加入后，负责人会把你拉进官方微信群。群里有活动通知、资源共享和日常闲聊。</div>
                    </div>
                </div>
                <div style="display:flex;align-items:flex-start;gap:12px;">
                    ...
                    <div>
                        <div style="font-size:13px;font-weight:700;color:#1a1a2e;margin-bottom:2px;">进群后建议做的事</div>
                        <div style="font-size:12px;color:rgba(26,26,46,0.45);line-height:1.5;">自我介绍时说说你为什么感兴趣；主动问问新人任务是什么；跟着参加前两次活动，陌生感会很快消失。</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    with col_r:
        ...
        intro = st.text_area("自我介绍 *",
                              ...
                              height=100, key="form_intro")
        time_avail = st.multiselect("你通常空闲的时间段",
                                    ["周一至周五 白天", "周一至周五 晚上", "周末全天", "周末上午",
                                     "周末下午"], key="form_time")
        ...
        all_filled = all(
            [name.strip(), student_id.strip(), major.strip(), grade != "请选择", phone.strip(),
             intro.strip()])

        if st.button("🎉  提交所有申请", key="apply_submit", type="primary", use_container_width=True,
        ...
        if not all_filled:
            st.markdown(
                '<div style="font-size:12px;color:rgba(26,26,46,0.3);text-align:center;margin-top:6px;">请填写所有必填项后提交</div>',
                unsafe_allow_html=True)
    ...
    with col:
        ...
        <div class="success-glass">
            ...
            <div style="background:rgba(255,255,255,0.7);border:1px solid rgba(99,102,241,0.1);border-radius:14px;padding:18px;text-align:left;margin-bottom:20px;">
                ...
                {''.join([f'<div style="display:flex;align-items:center;gap:8px;font-size:14px;color:rgba(26,26,46,0.75);margin-bottom:8px;"><span style="color:#15803d;">✓</span>{n}</div>' for n in club_names])}
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            '<div style="font-size:16px;font-weight:800;color:#1a1a2e;margin-bottom:12px;letter-spacing:-0.3px;">加入社团后，怎么快速融入？</div>',
            unsafe_allow_html=True)

        tips_cols = st.columns(3, gap="small")
        tips = [
            ("💬", "第一次活动前",
             "主动在群里自我介绍，说说你为什么加入。「你好，我是新来的xx，对xxx特别感兴趣」——比你想象的更有用。"),
            ("🤝", "第一个月",
             "每次活动尽量不缺席。陌生感主要来自不熟悉，前几次见面之后会好很多。遇到不懂的就问老成员。"),
            ("🌱", "稳定下来之后",
             "尝试承担一个具体的小任务或职位。有责任感的成员会被记住，也更容易从中获得真实的成长。"),
        ]
        for col, (icon, title, desc) in zip(tips_cols, tips):
            with col:
                st.markdown(f"""
                <div class="step-card">
        ...
        <div style="background:rgba(99,102,241,0.05);border:1px solid rgba(99,102,241,0.12);border-radius:14px;padding:16px 20px;margin-bottom:16px;">
            ...
            <div style="font-size:13px;color:rgba(26,26,46,0.6);line-height:1.65;">
                进群后记得设置「不消息免打扰」（至少前一个月）——很多活动通知和临时安排都在群里。
                群里通常有：活动通知、资源分享、作品交流、日常闲聊。
                不要只是「潜水」，哪怕发个表情包也比完全沉默好。
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def page_create():
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.markdown(
        '<div style="font-size:15px;color:rgba(26,26,46,0.45);line-height:1.7;margin-bottom:28px;">每一个存在的社团，都是某个人第一次说「我想做这件事」。<br>如果你有个想法，这里是它开始的地方。</div>',
        unsafe_allow_html=True)
    ...
    with col_l:
        ...
        club_type = st.selectbox("社团类型 *", ["请选择", "艺术创作", "音乐表演", "舞台表演", "科技创新",
                                               "商业创新", "户外运动", "人文学术", "公益服务", "体育竞技",
                                               "跨文化交流", "科学探索", "其他"], key="c_type")
        ...
        activity_plan = st.text_area("你打算做哪些活动或项目？",
                                      placeholder="大概说说你的活动构想，哪怕很初期的想法也可以", height=90,
                                      key="c_plan")
        target_members = st.slider("你预计招募多少初始成员？", 5, 50, 15, key="c_members")
        freq_plan = st.selectbox("计划的活动频率",
    ...
    with col_r:
        ...
        f_grade = st.selectbox("年级 *", ["请选择", "大一", "大二", "大三", "大四", "研究生"],
                                key="c_fgrade")
        f_phone = st.text_input("联系方式 *", key="c_fphone")
        f_exp = st.text_area("你有哪些相关经历或能力？（选填）",
                              placeholder="比如：你本身就是这个领域的爱好者？做过类似的组织工作？",
                              height=80, key="c_fexp")
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown(
            '<div class="form-section" style="margin-top:10px;"><div class="form-section-title">AI 帮你完善方案</div>',
            unsafe_allow_html=True)
        st.markdown(
            '<div style="font-size:13px;color:rgba(26,26,46,0.45);line-height:1.6;margin-bottom:14px;">填完基本信息后，可以让 AI 给你的创建方案提一些建议——对写申请书很有帮助。</div>',
            unsafe_allow_html=True)
...
请给出3条简短、具体、有用的建议，帮助他完善社团方案。风格温暖鼓励，直接分点，不用markdown标题。"""}]
                            )
                        ...
                        st.markdown(f'<div class="bubble-ai">✦ AI 建议：<br /><br />{suggestion}</div>',
                                    unsafe_allow_html=True)
                    except Exception:
        ...
        <div class="form-section" style="margin-top:10px;">
            ...
            <div style="font-size:13px;color:rgba(26,26,46,0.6);line-height:1.8;">
                <div style="margin-bottom:6px;"><span style="color:#6366F1;font-weight:700;">① 初步审核</span>（1-2 个工作日）</div>
                <div style="margin-bottom:6px;color:rgba(26,26,46,0.45);font-size:12px;padding-left:14px;">学生活动部确认材料完整，通过后进入答辩环节</div>
                <div style="margin-bottom:6px;"><span style="color:#6366F1;font-weight:700;">② 创建人答辩</span>（约 15 分钟）</div>
                <div style="margin-bottom:6px;color:rgba(26,26,46,0.45);font-size:12px;padding-left:14px;">向评审委员会说明社团定位、活动计划和初始招募方案</div>
                <div style="margin-bottom:6px;"><span style="color:#6366F1;font-weight:700;">③ 试运营期</span>（一学期）</div>
                <div style="color:rgba(26,26,46,0.45);font-size:12px;padding-left:14px;">完成 3 次以上有记录的活动后，正式注册挂牌，登上 ClubMatch 平台</div>
            </div>
        </div>
        """, unsafe_allow_html=True)

        all_ok = all([club_name, club_type != "请选择", club_desc, club_why, f_name, f_sid, f_major,
                      f_grade != "请选择", f_phone])
        if st.button("🚀  提交创建申请", key="c_submit", type="primary", use_container_width=True,
                     disabled=not all_ok):
        ...
        if not all_ok and not st.session_state.get("create_submitted"):
            st.markdown(
                '<div style="font-size:12px;color:rgba(26,26,46,0.28);text-align:center;margin-top:6px;">请填写所有必填项（*）</div>',
                unsafe_allow_html=True)
